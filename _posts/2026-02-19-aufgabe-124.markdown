...
const int SZ_sieve = 200'001; // because of padding need 1 more
...
vi createRads(const vvi& primeFacs) {
    int size = primeFacs.size();
    vi rads(size);
    rads[0] = 0;
    rads[1] = 1;
...
int main() {
    ...
    auto primeFacs = createPrimeFacs(SZ_sieve);
    auto rads = createRads(primeFacs);
...
Hat man [Laaksons Buch][laakson] gelesen (siehe auf S.44 f), dann weiß man, dass der g++-Compiler über ein paar eingebaute sogenannte policy-based data-sets (kurz "pbds") verfügt, auf die man hier zurückgreifen kann. Es ist natürlich nicht verboten, selbst einen Fenwick tree zu schreiben, auch dafür findet man genug Material im Internet. Ich begnüge mich einfach mit dem Einbinden der nötigen header und einem typedef:
...
Jetzt kann man mit .insert() ganz einfach Elemente hinzufügen. Sehr praktisch! Also füllen wir mal immer bis zum nächsten L auf und updaten den laufenden Wert von L:
...
Mit .find_by_order() erhält man einen Iterator auf das Element an einem bestimmten Index, wieder extrem praktisch, das ist genau was wir für diese Aufgabe brauchen!
